...
public class SecurityConfig {
    ...
    public InMemoryUserDetailsManager inMemoryUserDetailsManager(PasswordEncoder passwordEncoder) {
        return new InMemoryUserDetailsManager(
                ...
                User.withUsername("ons").password(passwordEncoder.encode("1234")).roles("USER","AGENT").build(),
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception {
        ...
        httpSecurity.authorizeRequests().requestMatchers("/savePlat").hasAnyRole("ADMIN","AGENT");
        httpSecurity.authorizeRequests().requestMatchers("/ListePlats").hasAnyRole("ADMIN","AGENT","USER");
        httpSecurity.authorizeRequests().requestMatchers("/supprimerPlat","/modifierPlat","/updatePlat").hasAnyRole("ADMIN");
